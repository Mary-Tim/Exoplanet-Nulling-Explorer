...
mars_location=np.array([-100., 112.]) / cons._radian_to_mas
...
class Amplitude(BaseAmplitude):
    ...
    def forward(self, data):
        return (self.earth(data) + self.mars(data) + self.star(data) + self.local_zodi(data) + \
...
amp.mars.radius.data = torch.tensor(mars_radius)
...
class DiffAmplitude(BaseAmplitude):
    def __init__(self):
        ...
        self.mars = get_amplitude('RelativePlanetBlackBody')()
        self.instrument = MiYinBasicType()

    def forward(self, data):
        return (self.earth(data) + self.mars(data)) * self.instrument(data)
...
bounds = [(0.1, 3.), (0.1, 3.), tuple(fov), tuple(fov), (0.1, 3.), (0.1, 3.), tuple(fov), tuple(fov)]
