...
fit_result.set_item('ra_grid', ra_grid.cpu().detach().numpy()*cons._radian_to_mas)
fit_result.set_item('dec_grid', dec_grid.cpu().detach().numpy()*cons._radian_to_mas)
fit_result.set_item('nll_grid', nll_grid)
fit_result.set_item('significance', significance)
fit_result.save(f'{output_path}/result_fitter.hdf5')
...
plt.savefig(f'{output_path}/fitter.pdf')
plt.show()