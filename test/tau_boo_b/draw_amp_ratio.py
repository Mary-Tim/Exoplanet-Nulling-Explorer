...
amp_template = {
    ...
    'Configuration':{
        'distance': 15.6521,         # distance between Miyin and target [pc]
        #'star_radius': 0.0002*cons._sun_radius,  # Star radius [kilometer]
        'star_radius': 1.4259*cons._sun_radius,  # Star radius [kilometer]
        'star_temperature': 6466.27,   # Star temperature [Kelvin]
        'target_longitude': 0.,     # Ecliptic longitude [degree]
        'target_latitude': 17.46,      # Ecliptic latitude  [degree]
...
print(f"Spec num: {obs_creator.spec_num}")
...
def draw_line(ax, amp_name):
    ...
    return [data_with_trans, data_0[0]]
...
if __name__ == '__main__':
    ...
    for key in amp_to_draw:
        lines[key] = draw_line(ax, key)
        
    x_array = lines['tau_Boo_b'][1]['wl_mid'].cpu().detach().numpy()*1e6 
    poxb_light = lines['tau_Boo_b'][0]
    star_light = lines['Stellar_Leak'][0] + lines['Local_zodi'][0] + lines['Exo_zodi'][0]
    #star_light = lines['Stellar_Leak'][0] + lines['Local_zodi'][0] + lines['Exo_zodi'][0]
    wl_width = (lines['tau_Boo_b'][1][0]['wl_hi'] - lines['tau_Boo_b'][1][0]['wl_lo']) * 1e6
    ax.plot(x_array, (poxb_light/star_light).cpu().detach().numpy(), linestyle=':', color='black')
    #SNR = poxb_light / torch.sqrt(poxb_light + star_light)
    #ax.plot(x_array, SNR.cpu().detach().numpy(), linestyle=':', color='black')
