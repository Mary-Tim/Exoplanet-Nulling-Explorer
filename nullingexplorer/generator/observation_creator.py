...
class ObservationCreator():
    ...
    def generate(self) -> TensorDict:
        ...
        data = TensorDict({
            ...
            'wl_lo': torch.tensor([np.repeat(self.__spec_gen.wl_lo, mod_num)]*obs_num).flatten(),
            'wl_hi': torch.tensor([np.repeat(self.__spec_gen.wl_hi, mod_num)]*obs_num).flatten(),
            'wl_mid': torch.tensor([np.repeat(self.__spec_gen.wl_mid, mod_num)]*obs_num).flatten(),
            'mod': torch.tensor(self.__obs_gen.mod*obs_num*spec_num).flatten(),
