...
class RelativePolarPlanetBlackBody(BaseAmplitude):
    ...
    def forward(self, data):
        # return the number of photons generated by the planet
        ra, dec = self.trans_map.to_cartesian(self.r_radius * self.e_rad, self.r_polor)
        return torch.pi * (self.r_radius * self.e_rad / (self.distance))**2 * self.spectrum(self.r_temperature * self.e_temp, data) * \
            self.trans_map(ra, dec, data['wl_mid'], data) * self.instrument.field_of_view(ra, dec, data['wl_mid'])

class PlanetCartesianCoordinates(BaseAmplitude):
...
class PlanetPolarCoordinates(BaseAmplitude):
    '''
    TODO: To be removed
    '''
    def __init__(self, spec='BinnedBlackBody'):
